...
const TEMPLATE_CONTENT: Record<TemplateId, TemplateContent> = {
  ...
  "5": {
    ...
    part1: `【*严禁*做可视化网站，你的目标是写一份报告，而不是做网站】根据我的要求，完成两大任务
（1）信息搜索
你现在是 SEO 大师，需要围绕关键词 [ {{keyword}} ] 为内容站 [  ] 写一篇内容页（我们站的功能：[ 提供网络安全的信息，仅此而已 ]）；但是需要你先搜集信息，主要是模拟真实用户在相关场景下带着自己的痛点去搜索信息。最终给我整理出一份报告，内容包括：
1. 什么样的人群在什么场景下会搜索这个词，核心痛点是什么（1-3 个）？
2. 同类型人的痛点还有什么？（2-4 个）
3. 在搜索结果中，排名前 5 的竞争对手他们满足了用户什么痛点？
4. 为了超越竞争对手的结果，还需要增加哪些内容？
...
每个章节标记目标关键词要多少次（不低于：[ 55 ] 次）。
每个章节标题包含相关关键词，且每一个章节用到的关键词（至少 5 个）你要列出（一定全部用到，不用列出目标关键词）：
（次关键词：[ {{secondary}} ]）
（LSI 关键词：[ {{lsi}} ]）
在大纲中标注需要插入的数据、案例
包含强有力的结论和行动号召
预计每部分字数（总计 2000-3500 字）
给我中文版即可
大纲格式要 SEO 友好，使用 H2、H3 标签结构
需要基于你调研的信息给我大纲
大纲内容侧重给出落地措施的角度，而不是描述大量的意义、定义这些（文章要注重怎么做）
...
【启动 Agent 模式】`,
    part2: `下面请你基于文章大纲，来做撰写文章，并插入引用，和提供元数据（title description）这三件事情：
（1）写正文的要求：
1. 确保正文总字数：[ 2000-3500 ] 字之间；（说的是正文，不包括引用链接的字数；允许 100-200 字符的差距）
2. 全部用英文；
3. 不要引用
4. 命中核心关键词 [ {{keyword}} ][ 55 ] 次、次要关键词、LSI 关键词；
